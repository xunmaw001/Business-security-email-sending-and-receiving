...
<div id="body_content">
    ...
    <#--<div class="button">标记为</div>-->
        <button type="button" class="btn btn-default" onclick="deleteMail('normal')">删除</button>
        ...
        <div class="btn-group">
            ...
            <div class="btn-group">
                ...
                <ul class="dropdown-menu">
                    <li><a href="" onclick="moveToSpam('normal','spam')">垃圾邮件</a></li>
                    <li><a href="" onclick="moveToSpam('normal','delete')">删除邮件</a></li>
                </ul>
            </div>
        </div>
    ...
    <#--</div>-->
        <div class="mail-list">
            ...
            <#--</thead>-->
                <tbody>
            <#if mailList??>
                <#list mailList?sort_by("sentDate")?reverse as mail>
                 <tr>
                     <td width="5%"><input type="checkbox" name="mailId" value="${(mail.id)!}"/></td>

                     <td width="15%"><a style="cursor: pointer"
                                        onclick="changeiframe('/mail/showMail',${(mail.id)!})"> ${(mail.fromUser)!}</a>
                     </td>
                     <td width="70%"><a style="cursor: pointer"
                                        onclick="changeiframe('/mail/showMail',${(mail.id)!})"> ${(mail.subject)!}</a>
                     </td>
                     <td width="10%"><a style="cursor: pointer"
                                        onclick="changeiframe('/mail/showMail',${(mail.id)!})">${(mail.sentDate?string("yyyy-MM-dd"))!}</a>
                     </td>
