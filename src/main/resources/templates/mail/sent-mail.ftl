    ...
    <#--</div>-->
        <div class="mail-list">
            ...
            <#--</thead>-->
                <tbody>
            <#if sentMailList??>
                <#list sentMailList as mail>
                 <tr>
                     <td width="5%"><input type="checkbox"/><input type="hidden" id="mailId" value="${(mail.id)!}"/>
                     </td>
                     <td width="15%"><a style="cursor: pointer"
                                        onclick="changeiframe('/mail/showSentMail',${(mail.id)!})"> ${(mail.toUser)!}</a>
                     </td>
                     <td width="70%"><a style="cursor: pointer"
                                        onclick="changeiframe('/mail/showSentMail',${(mail.id)!})"> ${(mail.subject)!}</a>
                     </td>
                     <td width="10%"><a style="cursor: pointer"
                                        onclick="changeiframe('/mail/showSentMail',${(mail.id)!})">${(mail.sentDate?string("yyyy-MM-dd"))!}</a>
                     </td>
